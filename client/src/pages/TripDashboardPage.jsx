...
export default function TripDashboardPage() {
  ...
  const allListsSelected = Boolean(lists.length) && selectedListIds.length === lists.length;
  const visibleIdeasLabel = useMemo(() => {
    if (!selectedListIds.length) {
      return "";
    }
    if (selectedListIds.length === 1) {
      return lists.find((list) => list.id === selectedListIds[0])?.name || "this list";
    }
    return "the selected lists";
  }, [lists, selectedListIds]);
  const emptyIdeasTitle = hasSelectedLists ? `Nothing in ${visibleIdeasLabel} yet` : "No lists selected";
  const emptyIdeasDescription = hasSelectedLists
    ? "Add a plan item or start from the recommended suggestions below."
    : "Select one or more lists to show items on the board and the map.";

  const getRecommendedListName = (recommendation) => {
    const exactMatch = lists.find((list) => slugify(list.name) === slugify(normalizedRecommendationSearch));
    if (exactMatch) {
    ...
    const foodLists = lists.filter((list) => looksFoodRelated(list.name));
    const placeLists = lists.filter((list) => slugify(list.name).includes("place") || slugify(list.name).includes("visit"));
    const activityLists = lists.filter((list) => !isPlaceLikeList(list.name));

    if (looksFoodRelated(contextText) && foodLists.length) {
      return foodLists[0].name;
    }

    if ((recommendation?.entryType === "activity" || looksActivityRelated(contextText)) && activityLists.length) {
      return activityLists[0].name;
    }

    if ((recommendation?.entryType === "place" || looksPlaceRelated(contextText)) && placeLists.length) {
      return placeLists[0].name;
    }

    if (recommendation?.entryType === "place") {
      return (placeLists[0] || foodLists[0] || lists[0])?.name || DEFAULT_RECOMMENDATION_SEARCH;
    }

    return (activityLists[0] || lists[0])?.name || DEFAULT_RECOMMENDATION_SEARCH;
  };

  const getRecommendationTargetListName = (recommendation) => {
    const recommendationKey = getRecommendationKey(recommendation);
    return recommendationTargetByKey[recommendationKey] || getRecommendedListName(recommendation);
  };

  useEffect(() => {
    const fallbackQuery = activeTrip?.destination?.mapQuery || activeTrip?.destination?.label || "";
    if (!activeMapQuery || activeMapQuery === fallbackQuery) {
      return;
    }

    const isVisibleOnMap = mappedIdeas.some((idea) => idea.mapQuery === activeMapQuery);
    if (!isVisibleOnMap) {
      setActiveMapQuery("");
    }
  }, [activeMapQuery, activeTrip?.destination?.label, activeTrip?.destination?.mapQuery, mappedIdeas]);
