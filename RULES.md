...
3. Never use local storage unless specifically requested by the user for some feature that requires temporary storage, always update through database calls by default
...
5. Any alerts or popups should use custom components that we have already created, not an alert()
6. User roles are specified in a table format as follows:
-- PERMISSION SUMMARY:
-- ┌──────────────┬───────────┬────────────┬──────────┐
